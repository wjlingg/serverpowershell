...
$numberofcores = (Get-WmiObject -class win32_processor -ComputerName $computername).NumberOfCores
$memoryram = (Get-CimInstance -ClassName Win32_ComputerSystem ).totalphysicalmemory / 1GB
$disksize = (Get-WmiObject -class win32_logicaldisk -ComputerName $computername).Size / 1GB
$freespace = (Get-WmiObject -class win32_logicaldisk -ComputerName $computername).FreeSpace / 1GB
